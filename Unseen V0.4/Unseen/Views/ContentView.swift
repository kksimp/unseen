...
// MARK: - Idea Naming View

struct IdeaNamingView: View {
    @EnvironmentObject var viewModel: GameViewModel
    @State private var ideaName: String = ""
    @State private var randomIndex: Int = Int.random(in: 0..<randomIdeaNames.count)
    @State private var showCharacterLimit = false
    @FocusState private var isTextFieldFocused: Bool
    
    var currentSuggestion: String {
        randomIdeaNames[randomIndex].name
    }
    ...
    var isValidLength: Bool {
        characterCount > 0 && characterCount <= maxIdeaNameLength
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            // Header
            HStack {
                Button(action: { viewModel.currentScreen = .difficultySelect }) {
            ...
            VStack(spacing: 32) {
                ...
                // Description
                VStack(spacing: 12) {
                    Text("What will you spread?")
                        .font(.system(size: 22, weight: .light))
                        .foregroundColor(.white)
                    
                    Text("Name the idea that will transform America.\nThe news will tell its story.")
                ...
                VStack(spacing: 8) {
                    ZStack {
                        RoundedRectangle(cornerRadius: 12)
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 12)
                                    .stroke(
                                        ideaName.isEmpty ? Color.white.opacity(0.15) :
                                            (isValidLength ? Color(red: 0.6, green: 0.5, blue: 0.8).opacity(0.5) : Color.red.opacity(0.5)),
                                        lineWidth: 1
                                    )
                            )
                        
                        TextField("", text: $ideaName)
                            .placeholder(when: ideaName.isEmpty) {
                            ...
                            .onChange(of: ideaName) { oldValue, newValue in
                                if newValue.count > maxIdeaNameLength {
                                    ideaName = String(newValue.prefix(maxIdeaNameLength))
                    ...
                    HStack {
                        Spacer()
                        Text("\(characterCount)/\(maxIdeaNameLength)")
                            .font(.system(size: 11, weight: .light, design: .monospaced))
                            .foregroundColor(characterCount > maxIdeaNameLength - 4 ? Color.orange.opacity(0.7) : Color.white.opacity(0.3))
                    }
                    ...
                    HStack(spacing: 12) {
                        Button(action: {
                            withAnimation(.easeInOut(duration: 0.2)) {
                                randomIndex = (randomIndex + 1) % randomIdeaNames.count
                            }
                        }) {
                            ...
                            Text(currentSuggestion)
                                ...
                                .background(
                                    RoundedRectangle(cornerRadius: 8)
                                        ...
                                        .overlay(
                                            RoundedRectangle(cornerRadius: 8)
                                                .stroke(Color(red: 0.6, green: 0.5, blue: 0.8).opacity(0.3), lineWidth: 1)
                                        )
                                )
                        }
                ...
                if !ideaName.isEmpty {
                    VStack(spacing: 8) {
                        ...
                        Text("\"\(ideaName)\" reaches 10% of Americans")
            ...
            VStack(spacing: 16) {
                Button(action: {
                    let name = ideaName.isEmpty ? currentSuggestion : ideaName
                    viewModel.setIdeaName(name)
                    viewModel.startGame()
                    ...
                    HStack(spacing: 10) {
                        Text(ideaName.isEmpty ? "USE SUGGESTION" : "BEGIN")
                            .font(.system(size: 14, weight: .medium))
                            .tracking(3)
                ...
                if ideaName.isEmpty {
                    Text("Tap suggestion to use it, or enter your own")
...
struct HowToPlayView: View {
    ...
    let pages: [(title: String, content: String, icon: String)] = [
        ("THE IDEA", "You control the spread of an idea across America. Name it. Watch it spread. See how the nation reacts.", "lightbulb.fill"),
    ...
    var body: some View {
        VStack(spacing: 40) {
            ...
            VStack(spacing: 20) {
                ...
                if let usa = viewModel.usa, let ideaName = usa.ideaName {
                    Text("\"\(ideaName.name)\"")
                        .font(.system(size: 18, weight: .medium))
                        .foregroundColor(Color(red: 0.6, green: 0.5, blue: 0.8))
                        .padding(.top, 8)
