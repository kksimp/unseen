...
struct CitySceneView: UIViewRepresentable {
    ...
    class Coordinator: NSObject {
        ...
        deinit { stopCameraLoop() }

        func startCameraLoop() {
            guard displayLink == nil else { return }
            targetX = cameraX; targetZ = cameraZ; targetZoom = currentZoom
        ...
        func stopCameraLoop() { displayLink?.invalidate(); displayLink = nil }
        ...
        @objc func handlePinch(_ gesture: UIPinchGestureRecognizer) {
            ...
            case .changed:
                ...
                targetZoom = max(minZoom, min(maxZoom, targetZoom / deltaScale))
                let pinchCenterNow = gesture.location(in: scnView)
                let worldNow = screenToWorld(pinchCenterNow, in: scnView)
                targetX += pinchAnchorWorld.x - worldNow.x
                targetZ += pinchAnchorWorld.z - worldNow.z
                clampTargets()
                gesture.scale = 1.0
            default: break
            }
        ...
        private func clampTargets() {
            ...
            targetZ = targetZ >= 0 ? min(targetZ, maxOffsetZDown) : max(targetZ, -maxOffsetZUp)
            targetZoom = max(minZoom, min(maxZoom, targetZoom))
        }

        private func clampCurrent() {
            cameraX = max(-maxOffsetX, min(maxOffsetX, cameraX))
            cameraZ = cameraZ >= 0 ? min(cameraZ, maxOffsetZDown) : max(cameraZ, -maxOffsetZUp)
