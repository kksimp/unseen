...
struct IdeaName {
    ...
    enum Style {
        case statement    // "Birds Aren't Real" - a claim, use as-is or lowercase
        case noun         // "Veganism", "The Movement" - a thing
        case theNoun      // "The Great Reset" - always has "the"
        case adjNoun      // "Flat Earth" - can add "theory" or "movement"
    }
    
    // Auto-detect style from the name
    init(_ name: String) {
        self.name = name.trimmingCharacters(in: .whitespaces)
        
        let lower = self.name.lowercased()
        
        // Detect style
        if lower.hasPrefix("the ") {
            self.style = .theNoun
        } else if lower.contains(" is ") || lower.contains(" are ") ||
                  lower.contains(" isn't ") || lower.contains(" aren't ") ||
                  lower.contains(" was ") || lower.contains(" were ") ||
                  lower.contains(" can ") || lower.contains(" should ") ||
                  lower.contains(" will ") || lower.contains(" must ") ||
                  lower.contains(" have ") || lower.contains(" has ") {
            self.style = .statement
        } else if self.name.contains(" ") {
            // Multi-word without verb - likely adjective + noun
            self.style = .adjNoun
        } else {
            // Single word
            self.style = .noun
    ...
    // For use as subject: "X is spreading"
    // "The Great Reset is spreading" / "Belief in birds aren't real is spreading"
    var asSubject: String {
        switch style {
        case .statement:
            return "Belief that \(name.lowercased())"
        case .theNoun:
            return name
        case .noun, .adjNoun:
            return "The \(name.lowercased()) movement"
        }
    }
    
    // For "people believe X" / "people embrace X"
    // "people believe birds aren't real" / "people embrace the Great Reset"
    var asBelief: String {
        switch style {
        case .statement:
        ...
        case .noun:
            return name.lowercased()
        case .adjNoun:
            return "the \(name.lowercased()) idea"
        }
    }
    
    /// For "talking about X" / "discussing X"
    /// "talking about birds aren't real" / "talking about the Great Reset"
    var asObject: String {
        switch style {
        case .statement:
            return "the idea that \(name.lowercased())"
        case .theNoun:
            return name.lowercased()
        case .noun, .adjNoun:
            return name.lowercased()
    ...
    /// For "X believers" / "X supporters"
    var believers: String {
        switch style {
        case .statement:
            return "those who believe \(name.lowercased())"
        case .theNoun:
            let withoutThe = String(name.dropFirst(4)) // Remove "The "
            return "\(withoutThe.lowercased()) believers"
        case .noun, .adjNoun:
            return "\(name.lowercased()) believers"
        }
    }
    
    /// Short reference for headlines: the name or quoted if statement
    var forHeadline: String {
        switch style {
        case .statement:
            return "\"\(name)\""
        case .theNoun, .noun, .adjNoun:
            return name
        }
    }
    
    /// Possessive form for "X's spread"
    var possessive: String {
        switch style {
        case .statement:
            return "The movement's"
        case .theNoun:
            let withoutThe = String(name.dropFirst(4))
            if withoutThe.hasSuffix("s") {
                return "\(name)'"
            }
            return "\(name)'s"
        case .noun, .adjNoun:
...
// MARK: - Curated Random Names with Known Good Grammar

let randomIdeaNames: [(name: String, style: IdeaName.Style)] = [
    // Statements (claims that are true/false)
    ("Birds Aren't Real", .statement),
    ("Fish Know Secrets", .statement),
    ("Sleep Is Optional", .statement),
    ("Breakfast Is A Lie", .statement),
    ("Mondays Are Cursed", .statement),
    ("Cats Control Us", .statement),
    ("Time Is Circular", .statement),
    ("Luck Is Real", .statement),
    ("Pain Is Growth", .statement),
    ("Less Is More", .statement),
    ("Silence Is Power", .statement),
    ("Change Is Coming", .statement),
    ("We Are Connected", .statement),
    ("Nothing Is Random", .statement),
    ("Trees Remember", .statement),
    
    // The + Noun (movements/concepts with "the")
    ("The New Dawn", .theNoun),
    ("The Great Awakening", .theNoun),
    ("The Silent Majority", .theNoun),
    ("The Third Way", .theNoun),
    ("The Reset", .theNoun),
    ("The Turning", .theNoun),
    ("The Quiet Revolution", .theNoun),
    ("The Open Path", .theNoun),
    
    // Single nouns (isms, movements)
    ("Minimalism", .noun),
    ("Neo-Agrarianism", .noun),
    ("Localism", .noun),
    ("Slow Living", .noun),
    ("Deep Ecology", .noun),
    ...
    ("Flat Earth", .adjNoun),
    ("Digital Detox", .adjNoun),
    ("Radical Honesty", .adjNoun),
    ("Quiet Quitting", .adjNoun),
    ("Soft Power", .adjNoun),
    ("Deep Work", .adjNoun),
    ("Slow Food", .adjNoun),
    ("Simple Living", .adjNoun),
]

// Helper to get a random suggestion
func getRandomIdeaSuggestion() -> String {
    return randomIdeaNames.randomElement()?.name ?? "The Movement"
}

let maxIdeaNameLength = 28
...
enum InfluenceVector: String, CaseIterable, Identifiable, Codable {
...
class StateRegion: Identifiable, ObservableObject {
    ...
    func statusMessage(ideaName: IdeaName?) -> String {
        guard let idea = ideaName else {
            return "Waiting for something to happen..."
        }
        
        if ideaAdoptionRate > 0.7 {
            return [
                "\(idea.asSubject) dominates here.",
                "Nearly everyone believes \(idea.asBelief).",
                "\(idea.believers.capitalized) are everywhere."
            ].randomElement()!
        } else if ideaAdoptionRate > 0.4 {
            return [
                "More people believe \(idea.asBelief) every day.",
                "\(idea.asSubject) is gaining momentum.",
                "\(idea.believers.capitalized) grow in number."
            ].randomElement()!
        } else if counterAdoptionRate > 0.5 {
            return [
                "Strong resistance to \(idea.asObject) here.",
                "Most here reject \(idea.asBelief).",
                "The backlash against \(idea.asObject) is fierce."
            ].randomElement()!
        } else if socialUnrest > 0.5 {
            return [
                "Tensions over \(idea.asObject) run high.",
                "Families are divided over \(idea.asBelief).",
                "Heated arguments about \(idea.asObject) daily."
            ].randomElement()!
        } else if ideaAdoptionRate > 0.1 {
            return [
                "People are starting to discuss \(idea.asObject).",
                "Early \(idea.believers) are emerging.",
                "\(idea.asSubject) is taking root."
            ].randomElement()!
        } else if echoChambered {
            return [
                "\(idea.believers.capitalized) here are protected from doubt.",
                "An echo chamber shields the faithful.",
                "True believers, insulated from opposition."
            ].randomElement()!
        } else {
            return [
                "No one here has heard about \(idea.asObject) yet.",
                "Life continues, unaware of \(idea.asObject).",
                "Untouched by \(idea.possessive.lowercased()) spread."
            ].randomElement()!
        }
    }
    
    var statusMessage: String {
        statusMessage(ideaName: nil)
    }
}
...
class USA: ObservableObject {
    ...
    @Published var ideaName: IdeaName?
    ...
    // Display name helper
    var ideaDisplayName: String {
        ideaName?.name ?? "The Idea"
    }
    ...
    func setIdeaName(_ name: String) {
        let trimmed = String(name.prefix(maxIdeaNameLength)).trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
            self.ideaName = IdeaName(trimmed)
        }
    }
