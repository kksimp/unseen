...
class City: ObservableObject {
    ...
    @Published var hasBeenSeeded: Bool = false  // NEW: Track if the city has been seeded at all
    ...
    init(scenario: CityScenario, gridSize: Int = 12) {
        self.scenario = scenario
        self.config = scenario.scenarioConfig
        self.gridSize = gridSize
        self.districts = []
        
        // Generate reproducible seed - handle negative hashValue safely
        let timestamp = UInt64(Date().timeIntervalSince1970 * 1000)
        let scenarioHash = UInt64(bitPattern: Int64(scenario.hashValue))
        self.citySeed = timestamp ^ scenarioHash
        srand48(Int(citySeed & 0x7FFFFFFFFFFFFFFF))
    ...
    private func determineDistrictType(x: Int, y: Int) -> DistrictType {
        ...
        case .hub:
            ...
            if distFromCenter <= 4 { return [.commercial, .residential].randomElement()! }
            return .residential
        case .grid:
            ...
            return .residential
        case .dense:
            if distFromCenter <= 3 { return .commercial }
            return [.residential, .commercial].randomElement()!
        case .industrial:
            if y < 4 { return .industrial }
            if y == 4 { return .commercial }
            return .residential
        case .mixed:
...
enum MutationTier: Int, CaseIterable {
    case early = 1
    case mid = 2
    case late = 3
    ...
    var label: String {
        switch self {
        case .early: return "BASIC"
        case .mid: return "ADVANCED"
        case .late: return "APEX"
        }
    }
    
    var color: (r: Double, g: Double, b: Double) {
        switch self {
        case .early: return (0.5, 0.6, 0.5)
        case .mid: return (0.6, 0.5, 0.8)
        case .late: return (1.0, 0.7, 0.3)
        }
    }
}
...
struct Mutation: Identifiable {
    ...
    enum MutationEffect {
        ...
        // New mid-game effects
        case nightStalker(Float)          // Bonus spread at night
        case panicHarvest(Float)          // Gain points from high-stress districts
        case silentCarrier(Float)         // Spread without increasing influence visibility
        case adaptiveResistance(Float)    // Reduce resistance over time in seeded areas
        // New late-game effects
        case cascadeEvent(Float)          // Chance for chain reaction spread
        case immunityPiercing(Float)      // Ignore a portion of resistance entirely
        case shadowNetwork(Float)         // Spread through non-adjacent high-influence districts
        case criticalMass(Float)          // Exponential spread bonus above 50% influence
        case systemicShock(Float)         // High influence causes stress in neighboring districts
        case eventHorizon(Float)          // Districts above 80% influence cannot lose influence
    }
}

let availableMutations: [Mutation] = [
    // EARLY TIER (costs 3-4)
    Mutation(name: "Thrives when ignored", description: "Spreads faster in low-suppression areas", effect: .stealthBoost(0.3), cost: 3, tier: .early),
    Mutation(name: "Feeds on routine", description: "Stronger spread during day cycles", effect: .spreadBoost(0.2), cost: 3, tier: .early),
    Mutation(name: "Unstable growth", description: "Faster but unpredictable spread", effect: .volatilityIncrease(0.5), cost: 3, tier: .early),
    Mutation(name: "Crowd seeker", description: "Spreads faster in dense areas", effect: .densityThriving(0.25), cost: 4, tier: .early),
    
    // MID TIER (costs 5-7)
    Mutation(name: "Exploits connections", description: "Crosses district boundaries easier", effect: .connectivityExploit(0.4), cost: 5, tier: .mid),
    Mutation(name: "Weakens defenses", description: "Reduces natural resistance over time", effect: .resistanceReduction(0.15), cost: 5, tier: .mid),
    Mutation(name: "Stress response", description: "Thrives in high-stress districts", effect: .stressFeeder(0.3), cost: 6, tier: .mid),
    Mutation(name: "Night crawler", description: "Dramatically increased spread at night", effect: .nightStalker(0.6), cost: 6, tier: .mid),
    Mutation(name: "Panic harvest", description: "Gain bonus points from stressed districts", effect: .panicHarvest(0.4), cost: 5, tier: .mid),
    Mutation(name: "Silent carrier", description: "Spread is less visible to detection", effect: .silentCarrier(0.35), cost: 7, tier: .mid),
    Mutation(name: "Erosion", description: "Continuously erode resistance in seeded areas", effect: .adaptiveResistance(0.02), cost: 6, tier: .mid),
    
    // LATE TIER (costs 8-12)
    Mutation(name: "Authority blind", description: "Partially ignores suppression effects", effect: .suppressionResist(0.2), cost: 8, tier: .late),
    Mutation(name: "Cascade trigger", description: "Chance for explosive chain reaction", effect: .cascadeEvent(0.15), cost: 9, tier: .late),
    Mutation(name: "Immunity piercing", description: "Bypasses a portion of all resistance", effect: .immunityPiercing(0.25), cost: 10, tier: .late),
    Mutation(name: "Shadow network", description: "Influence jumps to distant high-influence areas", effect: .shadowNetwork(0.2), cost: 9, tier: .late),
    Mutation(name: "Critical mass", description: "Exponential growth above 50% influence", effect: .criticalMass(0.5), cost: 11, tier: .late),
    Mutation(name: "Systemic shock", description: "High influence destabilizes neighbors", effect: .systemicShock(0.1), cost: 10, tier: .late),
    Mutation(name: "Event horizon", description: "Saturated districts cannot be cleansed", effect: .eventHorizon(0.8), cost: 12, tier: .late),
]
