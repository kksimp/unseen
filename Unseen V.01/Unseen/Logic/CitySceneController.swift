...
class CitySceneController: ObservableObject {
    ...
    private let maxPeoplePerDistrict = 10
    ...
    private func setupScene() {
        ...
        // Remove distance-based fog entirely
    ...
    func generateCity(from city: City) {
        ...
        createGround(size: Float(city.gridSize) * tileSize + 40, offset: offset)
    ...
    private func clearCity() {
        ...
        scene.rootNode.childNodes.filter { $0.name == "ground" || $0.name == "borderFog" || $0.name == "groundFog" }.forEach { $0.removeFromParentNode() }
    }
    
    private func createGround(size: Float, offset: Float) {
        // Main grass ground
        let groundGeo = SCNPlane(width: CGFloat(size), height: CGFloat(size))
        ...
        // Add fog/mist around borders
        addBorderFog(citySize: Float(6) * tileSize, offset: offset)
    }

    private func addBorderFog(citySize: Float, offset: Float) {
        let fogColor = UIColor(red: 0.6, green: 0.7, blue: 0.85, alpha: 1.0)
        let fogWidth: Float = 18.0
        let fogHeight: Float = 10.0
        let cityHalf = citySize / 2 + 2.0
        
        // Create gradient fog material for walls
        ...
        // Four sides of fog walls
        let sides: [(position: SCNVector3, rotation: Float)] = [
            (SCNVector3(0, fogHeight/2, -cityHalf - fogWidth/2 + 2), 0),
            (SCNVector3(0, fogHeight/2, cityHalf + fogWidth/2 - 2), Float.pi),
            (SCNVector3(-cityHalf - fogWidth/2 + 2, fogHeight/2, 0), Float.pi/2),
            (SCNVector3(cityHalf + fogWidth/2 - 2, fogHeight/2, 0), -Float.pi/2)
        ]
        
        for (position, rotation) in sides {
            let fogGeo = SCNPlane(width: CGFloat(citySize + fogWidth * 2), height: CGFloat(fogHeight))
        ...
        // Ground fog plane - radial gradient clear in center, foggy at edges
        let groundFogMaterial = SCNMaterial()
        groundFogMaterial.diffuse.contents = fogColor
        groundFogMaterial.transparent.contents = createRadialFogGradientImage()
        groundFogMaterial.transparencyMode = .rgbZero
        groundFogMaterial.isDoubleSided = true
        groundFogMaterial.writesToDepthBuffer = false
        
        let groundFogSize = citySize + fogWidth * 4
        let groundFogGeo = SCNPlane(width: CGFloat(groundFogSize), height: CGFloat(groundFogSize))
        groundFogGeo.firstMaterial = groundFogMaterial
        
        let groundFogNode = SCNNode(geometry: groundFogGeo)
        groundFogNode.eulerAngles.x = -.pi / 2
        groundFogNode.position.y = 0.08
        groundFogNode.renderingOrder = 99
        groundFogNode.name = "groundFog"
        scene.rootNode.addChildNode(groundFogNode)
    }
    ...
    private func createRadialFogGradientImage() -> UIImage {
        ...
        // FIXED: Clear in center (alpha 0), foggy at edges (alpha 1)
        let colors = [
            UIColor.black.cgColor,      // Center: fully transparent (shows through)
            UIColor.black.cgColor,      // Keep clear for most of city
            UIColor(white: 0.5, alpha: 1.0).cgColor,  // Start fading
    ...
    private func createDistrictNode(_ district: District, offset: Float) -> SCNNode {
        ...
        let buildingCount = Int(district.density * 6) + 2
    ...
    // MARK: - Detailed Procedural Buildings
    ...
    private func createDetailedPitchedRoof(width: Float, depth: Float) -> SCNNode {
        ...
        // Main roof body (simplified pitched)
    ...
    private func createDetailedIndustrialBuilding() -> SCNNode {
        ...
        // Main corrugated warehouse body
        let bodyGeo = SCNBox(width: CGFloat(width), height: CGFloat(height), length: CGFloat(depth), chamferRadius: 0)
    ...
    private func createDetailedCivicBuilding() -> SCNNode {
        ...
        // Entablature (horizontal piece above columns)
    ...
    private func createDetailedPediment(width: Float, height: Float) -> SCNNode {
        ...
        // Simplified triangular pediment using a box
    ...
    private func addFoliage(to node: SCNNode, district: District, innerSize: Float) {
        ...
        let treeCount: Int
        switch district.type {
        case .park: treeCount = Int.random(in: 10...18)
        case .residential: treeCount = Int.random(in: 4...8)
        case .civic: treeCount = Int.random(in: 2...4)
    ...
    private func createDetailedTree() -> SCNNode {
        ...
        switch treeType {
        case 0: // Deciduous (layered spheres)
    ...
    private func spawnInitialAgents() {
        ...
        spawnCarsOnRoads(count: city.gridSize * 2)
    }
    ...
    private func spawnCarsOnRoads(count: Int) {
        ...
        for _ in 0..<count {
            ...
            // FIXED: Car faces direction of road segment
    ...
    private func createCar() -> SCNNode {
        ...
        // Car body - length along Z axis (forward)
    ...
    private func startRoadDrivingAnimation(for car: SCNNode) {
        ...
        // FIXED: Car faces movement direction (Z is forward)
