            ...
            if viewModel.isPaused {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    if let seed = viewModel.city?.citySeed {
                        Text("Seed: \(String(seed, radix: 16).uppercased())")
        ...
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 16) {
                    ...
                    ForEach(viewModel.availableMutationsForPurchase) { mutation in
                        MutationCard(
                            mutation: mutation,
        ...
        Button(action: {
            ...
            .background(
                RoundedRectangle(cornerRadius: 8)
                    .fill(showPurchaseFeedback ? Color(red: 0.6, green: 0.5, blue: 0.8).opacity(0.3) : Color.white.opacity(canAfford ? 0.05 : 0.02))
                    .overlay(
                        RoundedRectangle(cornerRadius: 8)
                            .stroke(canAfford ? Color(red: 0.6, green: 0.5, blue: 0.8).opacity(0.3) : Color.white.opacity(0.05), lineWidth: 1)
                    )
            )
